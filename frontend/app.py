...
def get_filtered_movies(
    ...
    movies = recommender.model_df.copy()
    ...
    # Only movie filtering logic remains
    # sort
    if sort_by == "rating" and "vote_average" in movies.columns:
        movies = movies.sort_values("vote_average", ascending=False)
    elif sort_by == "popularity" and "popularity" in movies.columns:
        movies = movies.sort_values("popularity", ascending=False)
    elif sort_by == "recent" and "release_date" in movies.columns:
...
with st.sidebar:
    ...
    min_rating = st.slider(
        ...
        min_value=0.0,
        max_value=10.0,
        value=0.0,  # Lowered default to 0.0 for debug
        step=0.5,
...
if st.session_state.show_recommendations and st.session_state.favorites:
    ...
    try:
        ...
        rec_movies = recommender.model_df[recommender.model_df['title'].isin(results['title'].tolist())]
            ...
            for j, col in enumerate(cols):
                if i + j < len(results):
                    ...
                    with col:
                        ...
                        # Display movie card
                        match_pct = row['match_%'] if 'match_%' in row and not pd.isna(row['match_%']) else ''
                        rating = row['vote_average'] if 'vote_average' in row and not pd.isna(row['vote_average']) else ''
                        st.markdown(f"""
                            <div class="movie-card">
                                <img src=\"{poster_url}\" class=\"movie-poster\" alt=\"{row['title']}\">
                                <div class=\"movie-info\">
                                    <p class=\"movie-title\">{row['title']}</p>
                                    <p class=\"movie-rating\">⭐ {rating}{' | Match: ' + str(int(match_pct)) + '%' if match_pct != '' else ''}</p>
                                </div>
                            </div>
                        """, unsafe_allow_html=True)
